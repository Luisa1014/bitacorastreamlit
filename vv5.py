...
def save_to_blob(responses, image_path=None):
    container_name = 'registros'
    
    # Convertir las respuestas en una cadena de texto separada por comas
    responses_text = ', '.join([f"{key}: {value}" for key, value in responses.items()])
    
    # Guardar las respuestas en un archivo de texto
    text_blob_name = "Luisa Molina..txt"
    
    try:
        container_client = blob_service_client.get_container_client(container_name)
        
        # Guardar el archivo de texto en el blob
        ...
        st.success("Respuestas guardadas correctamente en formato .txt.")
        
        # Opcional: Guardar la imagen si está disponible
        if image_path:
            ...
            st.success("Imagen guardada correctamente en Blob Storage.")
...
if st.session_state.screen == "login":
    ...
    if not st.session_state.completed:
        fields = ["Compañía", "Especialidad", "Descripción de actividades realizadas", "Persona encargada", "Fecha de entrega actividad", "Estado de la actividad"]
        for field in fields:
